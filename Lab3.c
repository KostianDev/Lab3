...
void bezierCurve(SDL_Renderer *renderer, int x1, int x2, int x3, int x4, int y1, int y2, int y3, int y4);
...
l_list *drawGraph(SDL_Renderer *renderer, SDL_Window *window, matrix matrix, l_list *list_ptr, int isDir);
...
void drawUndirConnections(SDL_Renderer *renderer, l_list *node1, l_list *node2,
                          int r, int width, int height, int gap2);
...
int main(int argc, char *argv[]) {
    ...
    while (!quit) {
        ...
        list1_ptr = drawGraph(undirectedRenderer, undirectedWindow,
                              undirectedMatrix, list1_ptr, 0);
        list2_ptr = drawGraph(directedRenderer, directedWindow,
                              directedMatrix, list2_ptr, 1);
...
l_list *drawGraph(SDL_Renderer *renderer, SDL_Window *window, matrix matrix, l_list *list_ptr, int isDir) {
    ...
    int gap, gap1, gap2, gap3, r, key = 1, pos = 1, flag = 1, dir = 0;
        ...
        for (int j = 0; j < N; ++j) {
            if (matrix.matrix[i][j] == 1) {
                ...
                if (i == j) {
                    ...
                    if(isDir){
                        dir = 1;
                        drawDirConnections(renderer, node1, node2, r, dir, width, height, gap2);
                    } else {
                        matrix.matrix[j][i] = 0;
                        drawUndirConnections(renderer, node1, node2, r, width, height, gap2);
                    }
                    if (matrix.matrix[j][i] == 1 && isDir == 1) {
                        matrix.matrix[j][i] = 0;
                        dir = -1;
                        drawDirConnections(renderer, node2, node1, r, dir, width, height, gap2);
                    }
                }
            }
        }
...
void drawDirConnections(SDL_Renderer *renderer, l_list *node1, l_list *node2,
    ...
    if ((abs(node1->key - node2->key) == 1) ||
        ...
        if (startX == endX && (startX == gap + r || startX == width - gap - r)) {
            ...
            bezierCurve(renderer, startX + (int) ((double) r * cos(angle1)), mid1X,
